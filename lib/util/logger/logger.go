...
func NewFileHook(file string, flag int, chmod os.FileMode, fieldsOrder []string) (*FileHook, error) {
	...
	return &FileHook{logFile, flag, chmod, fileFormatter}, nil
}
...
func LogFileHook(log *logrus.Logger, logPath string) error {
	...
	if f, ok := log.Formatter.(*formatter.Formatter); ok {
		fieldsOrder = f.FieldsOrder
	}

	fhook, err := NewFileHook(
		filePath, os.O_CREATE|os.O_APPEND|os.O_RDWR, 0o666, fieldsOrder)
	if err != nil {
		return errors.Wrap(err, "LogFileHook err")
	}
	log.AddHook(fhook)
